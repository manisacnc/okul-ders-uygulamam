...
function cizMenu() {
  var pr = profilOku();
  var h = '<div class="baslik"><h1>📚 Okul Ders Uygulamam <span style="font-size:11px;color:#999;background:#f0f0f0;padding:2px 6px;border-radius:6px">v26</span></h1>';
...
function cizDers() {
  ...
  if (p.foto) {
    ...
    if (b.ozet) h += '<button class="kucuk-buton" style="background:#1f8a70" onclick="sesliOkuKonu(' + i + ')">🔊 Konuyu Dinle</button>';
...
function ttsYedekOku(parcalar) {
  ...
  _ttsParcaIdx = 0;
  function oku() {
    if (_ttsParcaIdx >= parcalar.length) { ttsDurumGoster('✅ Tamamlandı'); _ttsParcaIdx = 0; return; }
    try {
...
function internetSesCal(parcalar) {
  if (!parcalar || !parcalar.length) { ttsDurumGoster('⚠️ Okunacak metin yok'); return; }
...
function seslendir(metin, turkce) {
  ...
  // GARANTİLİ YOL: tum cihazlarda kullanici jesti icinde internet sesini baslat.
...
function sesliOkuDinleMetin() { seslendir(dinleSesMetni, true); }
  ...
  kutuSUPA.sinifGetir(kod).then(function (snf) {
    ...
    kutuSUPA.ogrenciListele(kod).then(function (ogrler) {
      ...
      h += '<div class="unit-butonlar" style="margin-bottom:10px">'
        ...
        + '</div>';
      h += '<div class="unit-butonlar unit-butonlar-mod" style="margin-bottom:10px;flex-wrap:wrap">'
        + '<button class="kucuk-buton" style="background:#2ecc71" onclick="supaYoklama(supaDeger(\'supaSinifKod\'))">📋 Yoklama</button>'
        + '<button class="kucuk-buton" style="background:#1f8a70" onclick="supaSınavlar(supaDeger(\'supaSinifKod\'))">🧮 Sınav / Ödev Not</button>'
        + '<button class="kucuk-buton" style="background:#ff9f43" onclick="supaSınıfRapor(supaDeger(\'supaSinifKod\'))">📊 Sınıf Raporu</button>'
...
function cizSbaglan() {
  ...
  h += '<div class="ozet-karti">'
    ...
    + '<input id="supaNo2" type="text" placeholder="Okul numaran" style="width:100%;padding:8px;border:1px solid #ccc;border-radius:8px;margin:4px 0">'
    + '<button class="kucuk-buton" style="background:#6a5cff" onclick="supaBaglan()">🔗 Kontrol Et ve Bağlan</button>'
    + '</div>';
  h += '<div class="ozet-karti">'
    + '<input id="supaVeliKod" type="text" placeholder="Veli erişim kodu" style="width:100%;padding:8px;border:1px solid #ccc;border-radius:8px;margin:4px 0">'
    + '<button class="kucuk-buton" style="background:#1f8a70" onclick="supaVeliGiris()">👨‍👩‍👧 Veli ile Gör</button>'
    + '</div>';
  h += '<div id="supaMesaj" style="display:none;border-radius:8px;padding:10px;margin-bottom:12px"></div>';
  ekran.innerHTML = h;
  supaDevamUyariGetir();
...
function supaVeliGiris() {
  ...
  if (!kod) { supaMesaj('Veli erişim kodu gir.', 'kirmizi'); return; }
  kutuSUPA.veliKodAra(kod).then(function (ogr) {
    if (!ogr) { supaMesaj('Bu kodla öğrenci bulunamadı.', 'kirmizi'); return; }
    supayaVeliRaporu(ogr);
  }).catch(function (e) { supaMesaj('Hata: ' + e.message, 'kirmizi'); });
}
function supaDevamUyariGetir() {
  var bagli = supaJsonOku('supaBagli');
  if (!bagli || !bagli.id) return;
  kutuSUPA.ogrenciAll(bagli.id).then(function (all) {
    var yoklar = (all.yoklama || []).filter(function (y) { return y.durum === 'Y'; });
    var el = document.getElementById('uyariRozeti');
    if (yoklar.length >= 3 && el) {
      el.innerHTML = '<div class="ozet-karti" style="background:#ffe3e3;border-color:#e05656;color:#bd2130">⚠️ Dikkat! <b>' + yoklar.length + '</b> izinsiz devamsızlığın var. Öğretmeninle konuşmayı unutma.</div>';
    }
    if (typeof supaPanelDoldur === 'function') supaPanelDoldur();
  }).catch(function () { if (typeof supaPanelDoldur === 'function') supaPanelDoldur(); });
}
function supaPanelDoldur() {
  var bagli = supaJsonOku('supaBagli');
  var el = document.getElementById('supaPanel');
  if (!bagli || !bagli.id || !el) return;
  Promise.all([
      ...
      var el2 = document.getElementById('supaPanel');
      if (el2) el2.innerHTML = '<div class="kucuk-not">Panel yüklenemedi.</div>';
    });
}
function supayaVeliRaporu(ogr) {
  Promise.all([kutuSUPA.sinifGetir(ogr.kod), kutuSUPA.notListele(ogr.id), kutuSUPA.detayListele(ogr.id), kutuSUPA.ogrenciAll(ogr.id)])
    .then(function (rz) {
      var snf = rz[0] || {}, notlar = rz[1] || [], rd = rz[2] || [], all = rz[3] || { yoklama: [], davranis: [] };
      var h = '<button class="geri" onclick="git(\'sbaglan\')">⬅ Geri</button>';
      h += '<div class="baslik"><h1>👨‍👩‍👧 Veli Görünümü</h1><p>' + esc(ogr.adsoyad) + ' · ' + esc((snf && snf.ad) || ogr.kod) + '</p></div>';
      var devY = all.yoklama.filter(function (y) { return y.durum === 'Y'; }).length;
      h += '<div class="ozet-karti" style="background:#f2f4fa">📅 Toplam yoklama: <b>' + all.yoklama.length + '</b> · İzinsiz: <b>' + devY + '</b><br>🏅 Davranış puanı: <b>' + (all.davranis || []).reduce(function (t, d) { return t + (d.puan || 0); }, 0) + '</b></div>';
      if (!notlar.length) h += '<div class="kucuk-not">Henüz not yok.</div>';
      else {
        h += '<div class="baslik"><h2>📝 Notlar</h2></div>';
        notlar.forEach(function (n) { h += '<div class="ozet-karti" style="margin-bottom:6px"><b>' + esc(n.ders || '') + '</b>: ' + esc(n.not_) + '</div>'; });
      }
      var devams = rd.filter(function (r) { return r.tip === 'devamsizlik'; });
      if (devams.length) {
        h += '<div class="baslik"><h2>📅 Devamsızlıklar</h2></div>';
        devams.forEach(function (r) { h += '<div class="ozet-karti" style="margin-bottom:6px">' + (r.tarih ? esc(r.tarih) : '') + (r.deger ? ' · ' + esc(r.deger) + ' gün' : '') + ' ' + esc(r.aciklama || '') + '</div>'; });
      }
      var other = rd.filter(function (r) { return r.tip !== 'devamsizlik'; });
      if (other.length) {
        h += '<div class="baslik"><h2>📚 Diğer kayıtlar</h2></div>';
        other.forEach(function (r) { h += '<div class="ozet-karti" style="margin-bottom:6px">' + ({ calisma: '📚', konu: '🎯' }[r.tip] || '▪') + ' <b>' + esc(r.alan || r.tip) + '</b>' + (r.deger ? ' → ' + esc(r.deger) : '') + ' ' + esc(r.aciklama || '') + '</div>'; });
      }
      h += '<div style="margin-top:10px;text-align:center"><button class="btn btn-mor" onclick="window.print()">🖨️ Yazdır</button></div>';
      ekran.innerHTML = h;
    }).catch(function (e) { supaMesaj('Hata: ' + e.message, 'kirmizi'); });
}
function supaVeliKodUret(ogrenciId) {
  var kod = '';
  var alfabe = 'ABDEFGHJKLMNPRSTUVYZ';
  while (kod.length < 6) kod += alfabe.charAt(Math.floor(Math.random() * alfabe.length));
  kutuSUPA.veliKodAyarla(ogrenciId, kod).then(function () {
...
function supaBaglan() {
  ...
  kutuSUPA.ogrencietkinlesme(kod).then(function (ogr) {
    ...
    kutuSUPA.sinifGetir(ogr.kod).then(function (snf) {
      var snfAd = (snf && snf.ad) || ogr.kod;
      var h = '<button class="geri" onclick="git(\'sbaglan\')">⬅ Geri</button>';
      h += '<div class="baslik"><h1>🔍 Bu sen misin?</h1><p>Kod doğrulandı. Bilgilerini kontrol et.</p></div>';
      h += '<div class="ozet-karti" style="background:#f2f4fa">👤 <b>' + esc(ogr.adsoyad) + '</b><br>🏫 Sınıf: <b>' + esc(snfAd) + '</b><br>🔢 Okul no: <b>' + esc(ogr.no || '(belirtilmemiş)') + '</b></div>';
      h += '<div class="ozet-karti">Okul numaranı doğrula:<br><input id="supaNoKontrol" type="text" placeholder="Okul numaran" style="width:100%;padding:8px;border:1px solid #ccc;border-radius:8px;margin:4px 0">'
        + '<button class="kucuk-buton" style="background:#1f8a70" onclick="supaNoDogrula(' + ogr.id + ',\'' + encodeURIComponent(ogr.no || '') + '\',\'' + encodeURIComponent(ogr.kod) + '\',\'' + encodeURIComponent(ogr.adsoyad) + '\',\'' + encodeURIComponent(snfAd) + '\')">✅ Evet, benim</button>'
        + ' <button class="kucuk-buton" style="background:#e05656" onclick="git(\'sbaglan\')">❌ Hayır</button></div>';
      h += '<div id="supaMesaj" style="display:none;border-radius:8px;padding:10px;margin-bottom:12px"></div>';
      ekran.innerHTML = h;
    });
  }).catch(function (e) { supaMesaj('Hata: ' + e.message, 'kirmizi'); });
}
function supaNoDogrula(ogrId, kayitNoEnc, kodEnc, adsoyadEnc, snfAdEnc) {
  var kayitNo = kayitNoEnc ? decodeURIComponent(kayitNoEnc) : '';
  var kod = decodeURIComponent(kodEnc || '');
  var adsoyad = decodeURIComponent(adsoyadEnc || '');
  var snfAd = decodeURIComponent(snfAdEnc || '');
  var el = document.getElementById('supaNoKontrol');
  var girilen = el ? el.value.trim() : '';
  if (!girilen) { supaMesaj('Okul numaranı gir.', 'kirmizi'); return; }
  if (kayitNo && String(kayitNo).trim() !== girilen) { supaMesaj('✖️ Girilen numara kayıtlı numarayla eşleşmiyor. Tekrar dene ya da öğretmenine sor.', 'kirmizi'); return; }
  supaJsonYaz('supaBagli', { id: ogrId, no: girilen, adsoyad: adsoyad, kod: kod, sinifAd: snfAd });
  supaMesaj('✅ Sınıfa bağlandın: ' + snfAd, 'yesil');
  cizSbaglan();
}
...
/* ---- ÖĞRENCİ: Sınıf duyuruları ---- */
function supaDuyurularimGoster() {
  var bagli = supaJsonOku('supaBagli');
  if (!bagli || !bagli.id) { alert('Önce sınıfa bağlan.'); return; }
  Promise.all([kutuSUPA.duyuruListele(bagli.kod), kutuSUPA.okunmaDurumu(bagli.id)])
    .then(function (rz) {
      var list = rz[0] || [];
      var okunan = rz[1] || [];
      var okSet = {}; okunan.forEach(function (d) { okSet[d] = true; });
      var h = '<button class="geri" onclick="git(\'sbaglan\')">⬅ Geri</button>';
      h += '<div class="baslik"><h1>📢 Sınıf Duyuruları</h1><p>' + esc(bagli.sinifAd || bagli.kod) + '</p></div>';
      h += '<div id="supaMesaj" style="display:none;border-radius:8px;padding:10px;margin-bottom:12px"></div>';
      if (!list || !list.length) h += '<div class="kucuk-not">Henüz duyuru yok.</div>';
      else list.forEach(function (d) {
        var okudu = okSet[d.id];
        h += '<div class="ozet-karti" style="margin-bottom:6px"><b>📢 ' + esc(d.baslik) + '</b><br>' + esc(d.metin || '') + '<br><small style="color:#888">' + (d.olusturma ? new Date(d.olusturma).toLocaleString('tr-TR') : '') + '</small><br>'
          + (okudu
              ? '<button class="kucuk-buton" style="background:#2ecc71" disabled>✅ Okundu</button>'
              : '<button class="kucuk-buton" style="background:#1f8a70" onclick="supaOkuduIsaretle(' + d.id + ')">Okudum ✅</button>')
          + '</div>';
      });
      ekran.innerHTML = h;
    }).catch(function (e) { alert('Hata: ' + e.message); });
}
function supaOkuduIsaretle(duyuruId) {
  var bagli = supaJsonOku('supaBagli');
  if (!bagli || !bagli.id) { alert('Önce sınıfa bağlan.'); return; }
  kutuSUPA.okunmaIsaretle(duyuruId, bagli.id)
    .then(function () { supaMesaj('✅ Okundu olarak işaretlendi.', 'yesil'); supaDuyurularimGoster(); })
    .catch(function (e) { alert('Hata: ' + e.message); });
}
...
function supaNotlarimGoster() {
  var bagli = supaJsonOku('supaBagli');
  if (!bagli || !bagli.id) { alert('Önce sınıfa bağlan.'); return; }
  Promise.all([kutuSUPA.notListele(bagli.id), kutuSUPA.detayListele(bagli.id), kutuSUPA.ogrenciAll(bagli.id)])
    .then(function (rz) {
      var notlar = rz[0] || [], rd = rz[1] || [], all = rz[2] || { yoklama: [], davranis: [] };
      var h = '<button class="geri" onclick="git(\'sbaglan\')">⬅ Geri</button>';
      h += '<div class="baslik"><h1>📝 Notlarım</h1><p>Öğretmeninin senin için yazdığı tüm geri bildirimler.</p></div>';
      h += '<div id="supaMesaj" style="display:none;border-radius:8px;padding:10px;margin-bottom:12px"></div>';
      if (!notlar.length) h += '<div class="kucuk-not">Henüz not yok.</div>';
      else {
      ...
      if (devam.length) {
        h += '<div class="baslik" style="margin-top:10px"><h2>📅 Devamsızlıkların</h2></div>';
        devam.forEach(function (r) {
          h += '<div class="ozet-karti" style="margin-bottom:6px">📅 <b>' + (r.tarih ? esc(r.tarih) : 'tarihsiz') + '</b>' + (r.deger ? ' · <b>' + esc(r.deger) + '</b> gün' : '') + ' ' + esc(r.aciklama || '') + '</div>';
        });
      }
      var calisma = rd.filter(function (r) { return r.tip === 'calisma'; });
      if (calisma.length) {
        h += '<div class="baslik" style="margin-top:10px"><h2>📚 Çalışmaların</h2></div>';
        calisma.forEach(function (r) {
          h += '<div class="ozet-karti" style="margin-bottom:6px">📚 <b>' + esc(r.alan || r.deger || '') + '</b>' + (r.deger && r.alan ? ' · ' : '') + (r.alan ? esc(r.deger || '') : '') + ' ' + esc(r.aciklama || '') + '<br><small style="color:#888">' + (r.tarih ? esc(r.tarih) : '') + '</small></div>';
        });
      }
      var konu = rd.filter(function (r) { return r.tip === 'konu'; });
      if (konu.length) {
        h += '<div class="baslik" style="margin-top:10px"><h2>🎯 Konu Durumların</h2></div>';
        konu.forEach(function (r) {
          h += '<div class="ozet-karti" style="margin-bottom:6px">🎯 <b>' + esc(r.alan || '') + '</b> · ' + esc(r.deger || '') + ' ' + esc(r.aciklama || '') + '<br><small style="color:#888">' + (r.tarih ? esc(r.tarih) : '') + '</small></div>';
        });
      }
      var dv = all.davranis || [];
      if (dv.length) {
        var toplam = 0;
        dv.forEach(function (d) { toplam += (d.puan || 0); });
        h += '<div class="baslik" style="margin-top:10px"><h2>🏅 Davranış Puanın</h2></div>';
        h += '<div class="ozet-karti" style="background:#fff8e6">Toplam: <b>' + toplam + '</b> puan (' + dv.length + ' kayıt)</div>';
      }
      if (!notlar.length && !devam.length && !calisma.length && !konu.length && !dv.length) {
        h += '<div class="kucuk-not">Öğretmenin henüz geri bildirim yazmadı.</div>';
      }
      ekran.innerHTML = h;
    }).catch(function (e) { alert('Hata: ' + e.message); });
}
...
function supaDosyaGoster(ogrenciId, adEnc) {
  ...
  Promise.all([kutuSUPA.notListele(p.id), kutuSUPA.detayListele(p.id), kutuSUPA.ogrenciGetir(p.id)])
    .then(function (rz) {
      ...
      h += '<div class="baslik" style="margin-top:10px"><h2>👨‍👩‍👧 Veli Erişimi</h2></div>';
      h += '<div class="ozet-karti">Veli bu kodu girerek çocuğunun dosyasını <b>okuyabilir</b> (notlar, devamsızlık, çalışmalar, davranış).'
...
/* =========== SINIF YÖNETİMİ MODÜLLERİ =========== */

/* ---- YOKLAMA ---- */
function bugunTarih() { return new Date().toISOString().slice(0, 10); }
function supaYoklama(kod) {
  if (!kod) return;
  Promise.all([kutuSUPA.ogrenciListele(kod), kutuSUPA.yoklamaTarihe(bugunTarih())])
    .then(function (rz) {
      var ogrler = rz[0] || [];
      var yoklar = rz[1] || [];
      var yMap = {}; yoklar.forEach(function (y) { yMap[y.ogrenci_id] = y; });
      var h = '<button class="geri" onclick="git(\'sinifYonet\')">⬅ Sınıf</button>';
      h += '<div class="baslik"><h1>📋 Yoklama — ' + bugunTarih() + '</h1><p>Sınıf: <b>' + esc(kod) + '</b> · Her öğrenciye dokunup durum seç.</p></div>';
      h += '<div id="supaMesaj" style="display:none;border-radius:8px;padding:10px;margin-bottom:12px"></div>';
      if (!ogrler.length) { h += '<div class="kucuk-not">Öğrenci yok.</div>'; ekran.innerHTML = h; return; }
      var ht = { H: 0, G: 0, I: 0, Y: 0 };
      ogrler.forEach(function (o) { var d = yMap[o.id] ? yMap[o.id].durum : ''; if (ht[d] !== undefined) ht[d]++; });
      h += '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:10px">'
        + '<span style="background:#d4edda;color:#155724;padding:4px 10px;border-radius:8px">✅ Hazır: ' + ht.H + '</span>'
        + '<span style="background:#fff3cd;color:#856404;padding:4px 10px;border-radius:8px">⏱ Geç: ' + ht.G + '</span>'
        + '<span style="background:#d1ecf1;color:#0c5460;padding:4px 10px;border-radius:8px">📝 İzinli: ' + ht.I + '</span>'
        + '<span style="background:#f8d7da;color:#721c24;padding:4px 10px;border-radius:8px">❌ Yok: ' + ht.Y + '</span>'
        + '</div>';
      h += '<div style="display:flex;flex-direction:column;gap:8px">';
      ogrler.forEach(function (o) {
        var y = yMap[o.id]; var cur = y ? y.durum : '';
        var renk = cur === 'H' ? '#2ecc71' : cur === 'G' ? '#f39c12' : cur === 'I' ? '#3498db' : cur === 'Y' ? '#e74c3c' : '#dfe4f5';
        h += '<div style="display:flex;align-items:center;gap:6px;border:2px solid ' + renk + ';border-radius:10px;padding:8px">'
          + '<b style="flex:1">' + esc(o.adsoyad) + '</b>'
          + '<button class="kucuk-buton" style="background:' + (cur === 'H' ? '#1e7e34' : '#6c757d') + '" onclick="supaYokla(\'' + kod + '\',' + o.id + ',\'H\')">✅ Hazır</button>'
          + '<button class="kucuk-buton" style="background:' + (cur === 'G' ? '#d4a017' : '#6c757d') + '" onclick="supaYokla(\'' + kod + '\',' + o.id + ',\'G\')">⏱ Geç</button>'
          + '<button class="kucuk-buton" style="background:' + (cur === 'I' ? '#117a8b' : '#6c757d') + '" onclick="supaYokla(\'' + kod + '\',' + o.id + ',\'I\')">📝 İzinli</button>'
          + '<button class="kucuk-buton" style="background:' + (cur === 'Y' ? '#bd2130' : '#6c757d') + '" onclick="supaYokla(\'' + kod + '\',' + o.id + ',\'Y\')">❌ Yok</button>'
          + '</div>';
      });
      h += '</div>';
      h += '<div style="margin-top:12px;display:flex;gap:6px;flex-wrap:wrap"><button class="kucuk-buton" style="background:#0984e3" onclick="supaYoklamaCSV(supaDeger(\'supaSinifKod\'))">⬇️ CSV İndir</button><button class="kucuk-buton" style="background:#6a5cff" onclick="window.print()">🖨️ Yazdır</button></div>';
      ekran.innerHTML = h;
    }).catch(function (e) { alert('Hata: ' + e.message); });
}
function supaYokla(kod, ogrenciId, durum) {
  kutuSUPA.yoklamaKaydet(ogrenciId, bugunTarih(), durum)
    .then(function () { supaYoklama(kod); })
    .catch(function (e) { alert('Hata: ' + e.message); });
}

/* ---- CSV / EXCEL dışa aktarma ---- */
function supayaIndir(dosyaAd, satirlar, baslik) {
  if (!satirlar || !satirlar.length) { alert('Dışa aktarılacak veri yok.'); return; }
  var esc = function (v) { v = String(v == null ? '' : v); return '"' + v.replace(/"/g, '""') + '"'; };
  var csv = '';
  if (baslik) csv += baslik.map(esc).join(';') + '\n';
  satirlar.forEach(function (r) { csv += r.map(esc).join(';') + '\n'; });
  var blob = new Blob(['\ufeff' + csv], { type: 'text/csv;charset=utf-8;' });
  var url = URL.createObjectURL(blob);
  var a = document.createElement('a');
  a.href = url; a.download = dosyaAd;
  document.body.appendChild(a); a.click(); document.body.removeChild(a);
  setTimeout(function () { URL.revokeObjectURL(url); }, 3000);
}
function supaYoklamaCSV(kod) {
  Promise.all([kutuSUPA.ogrenciListele(kod), kutuSUPA.yoklamaTarihe('')]).then(function (rz) {
    var ogrler = rz[0] || [], yoklar = rz[1] || [];
    var satirlar = ogrler.map(function (o) {
      var kayitlar = yoklar.filter(function (y) { return y.ogrenci_id === o.id; });
      var durumlar = kayitlar.map(function (y) {
        var d = { H: 'Hazır', G: 'Geç', I: 'İzinli', Y: 'Yok' }[y.durum] || y.durum;
        return y.tarih + ':' + d;
      }).join(' | ');
      return [o.no || '', o.adsoyad, durumlar];
    });
    supayaIndir('yoklama-' + kod + '.csv', satirlar, ['No', 'Öğrenci', 'Yoklamalar (tarih:durum)']);
  }).catch(function (e) { alert('Hata: ' + e.message); });
}
function supaSinavCSV(kod) {
  Promise.all([kutuSUPA.sinavListele(kod), kutuSUPA.ogrenciListele(kod)]).then(function (rz) {
    var snv = rz[0] || [], ogrler = rz[1] || [];
    if (!snv.length) { alert('Sınav/ödev yok.'); return; }
    var beklenti = snv.map(function (s) { return kutuSUPA.sinavNotListele(s.id).then(function (n) { return { s: s, n: n || [] }; }); });
    Promise.all(beklenti).then(function (diziler) {
      var nMap = {}; diziler.forEach(function (d) {
        d.n.forEach(function (n) { nMap[n.ogrenci_id + '_' + d.s.id] = n.not_; });
      });
      var satirlar = ogrler.map(function (o) {
        var satir = [o.no || '', o.adsoyad];
        diziler.forEach(function (d) { satir.push(nMap[o.id + '_' + d.s.id] != null ? nMap[o.id + '_' + d.s.id] : ''); });
        return satir;
      });
      supayaIndir('sinav-notlar-' + kod + '.csv', satirlar, ['No', 'Öğrenci'].concat(diziler.map(function (d) { return d.s.ad + (d.s.tur && d.s.tur !== 'Sınav' ? ' (' + d.s.tur + ')' : ''); })));
    });
  }).catch(function (e) { alert('Hata: ' + e.message); });
}

/* ---- SINAV / ÖDEV NOT GİRİŞİ ---- */
function supaSınavlar(kod) {
  if (!kod) return;
  Promise.all([kutuSUPA.sinavListele(kod), kutuSUPA.ogrenciListele(kod)])
    .then(function (rz) {
      var snv = rz[0] || [];
      var h = '<button class="geri" onclick="git(\'sinifYonet\')">⬅ Sınıf</button>';
      h += '<div class="baslik"><h1>🧮 Sınav / Ödev Notları</h1><p>Sınav aç, tek ekrandan herkese not yaz.</p></div>';
      h += '<div class="ozet-karti">';
      h += '<input id="snAd" type="text" placeholder="Sınav / ödev adı (örn. 1. Yazılı Matematik)" style="padding:8px;border:1px solid #ccc;border-radius:8px;margin:4px 0;width:100%">';
      h += '<select id="snTur" style="padding:8px;border:2px solid #dfe5f0;border-radius:8px"><option>Sınav</option><option>Ödev</option><option>Quiz</option><option>Proje</option></select> ';
      h += '<input id="snTarih" type="date" style="padding:6px">';
      h += '<button class="kucuk-buton" style="background:#1f8a70" onclick="supaSınavYeni(\'' + kod + '\')">➕ Sınav Aç</button></div>';
      if (!snv.length) h += '<div class="kucuk-not">Henüz sınav/ödev yok.</div>';
      ...
      h += '<div style="margin-top:12px;display:flex;gap:6px;flex-wrap:wrap"><button class="kucuk-buton" style="background:#0984e3" onclick="supaSinavCSV(supaDeger(\'supaSinifKod\'))">⬇️ CSV İndir</button></div>';
      ekran.innerHTML = h;
    }).catch(function (e) { alert('Hata: ' + e.message); });
}
function supaSınavYeni(kod) {
  var ad = document.getElementById('snAd');
  var tur = document.getElementById('snTur');
  var tar = document.getElementById('snTarih');
  if (!ad || !ad.value.trim()) { alert('Sınav adı gir.'); return; }
...
function supaSınavNotGir(sinavId, adEnc) {
  ...
  kutuSUPA.ogrenciListele(supaDeger('supaSinifKod')).then(function (ogrler) {
    kutuSUPA.sinavNotListele(sinavId).then(function (notlar) {
      ...
      var h = '<button class="geri" onclick="supaSınavlar(supaDeger(\'supaSinifKod\'))">⬅ Geri</button>';
      h += '<div class="baslik"><h1>📝 Not Girişi — ' + esc(ad) + '</h1></div>';
      var toplam = 0, adet = 0;
      (ogrler || []).forEach(function (o) {
        var mevcut = nm[o.id];
        var deger = mevcut ? mevcut.not_ : '';
        var sayi = parseFloat(deger.replace(',', '.'));
        if (!isNaN(sayi)) { toplam += sayi; adet++; }
        h += '<div style="display:flex;align-items:center;gap:6px;border:1px solid #dfe4f5;border-radius:8px;padding:6px;margin-bottom:6px">'
          + '<b style="flex:1">' + esc(o.adsoyad) + '</b>'
          + '<input id="sn_' + o.id + '" type="text" inputmode="decimal" placeholder="Not" value="' + esc(deger) + '" style="width:90px;padding:6px;border:1px solid #ccc;border-radius:8px">'
          + '<button class="kucuk-buton" style="background:#1f8a70" onclick="supaSınavNotKaydet(' + sinavId + ',' + o.id + ')">💾</button>'
          + '</div>';
      });
      h += '<div class="ozet-karti">Sınıf ortalaması: <b>' + (adet ? (toplam / adet).toFixed(2) : '—') + '</b> (' + adet + ' not)</div>';
      h += '<div style="margin-top:10px;text-align:right"><button class="btn btn-mor" onclick="alertsTopluKaydet(' + sinavId + ')">Toplu Kaydet (hepsi)</button></div>';
      ekran.innerHTML = h;
    }).catch(function (e) { alert('Hata: ' + e.message); });
  }).catch(function (e) { alert('Hata: ' + e.message); });
}
function supaSınavNotKaydet(sinavId, ogrenciId) {
  var el = document.getElementById('sn_' + ogrenciId);
...
function alertsTopluKaydet(sinavId) {
  var chars = document.querySelectorAll('[id^="sn_"]');
  var n = 0;
  chars.forEach(function (el, i) {
    var id = parseInt(el.id.replace('sn_', ''), 10);
    if (!id) return;
    var deger = el.value.trim();
    if (!deger) return;
    kutuSUPA.sinavNotKaydet(sinavId, id, deger).then(function () { n++; }).catch(function (e) { });
  });
  alert('Kaydedildi.');
}

/* ---- DAVRANIŞ PUANI ---- */
function supaDavranış(kod) {
  if (!kod) return;
  kutuSUPA.ogrenciListele(kod).then(function (ogrler) {
    var h = '<button class="geri" onclick="git(\'sinifYonet\')">⬅ Sınıf</button>';
    h += '<div class="baslik"><h1>🏅 Davranış Puanı</h1><p>Öğrenci seç, puan ekle (✓ pozitif, ✗ negatif).</p></div>';
    h += '<div id="supaMesaj" style="display:none;border-radius:8px;padding:10px;margin-bottom:12px"></div>';
    (ogrler || []).forEach(function (o) {
      h += '<div style="display:flex;align-items:center;gap:6px;border:1px solid #dfe4f5;border-radius:8px;padding:6px;margin-bottom:6px;flex-wrap:wrap">'
        + '<b style="flex:1">' + esc(o.adsoyad) + '</b>'
        + '<button class="kucuk-buton" style="background:#2ecc71" onclick="supaDavranışKaydet(\'' + kod + '\',' + o.id + ', 3)">+3</button>'
        + '<button class="kucuk-buton" style="background:#2ecc71" onclick="supaDavranışKaydet(\'' + kod + '\',' + o.id + ', 1)">+1</button>'
        + '<button class="kucuk-buton" style="background:#e67e22" onclick="supaDavranışKaydet(\'' + kod + '\',' + o.id + ', -1)">-1</button>'
        + '<button class="kucuk-buton" style="background:#e05656" onclick="supaDavranışKaydet(\'' + kod + '\',' + o.id + ', -3)">-3</button>'
        + '</div>';
    });
    ekran.innerHTML = h;
  }).catch(function (e) { alert('Hata: ' + e.message); });
}
function supaDavranışKaydet(kod, ogrenciId, puan) {
  kutuSUPA.davranisKaydet(ogrenciId, null, puan)
    .then(function () { supaMesaj('Puan eklendi: ' + (puan > 0 ? '+' : '') + puan, 'yesil'); supaDavranış(kod); })
    .catch(function (e) { alert('Hata: ' + e.message); });
}

/* ---- RASTGELE ÖĞRENCİ SEÇ ---- */
function supaRastgele(kod) {
  if (!kod) return;
  kutuSUPA.ogrenciListele(kod).then(function (ogrler) {
    var h = '<button class="geri" onclick="git(\'sinifYonet\')">⬅ Sınıf</button>';
    h += '<div class="baslik"><h1>🎲 Rastgele Öğrenci Seç</h1><p>Adil cevaplayıcı seçimi için.</p></div>';
    h += '<div class="ozet-karti" style="text-align:center;padding:20px">';
    h += '<div id="rsSonuc" style="font-size:22px;font-weight:800;margin:10px 0;min-height:60px">—</div>';
    h += '<button class="btn btn-mor" onclick="supaRastgeleCek(\'' + kod + '\')">🎲 Çek</button></div>';
    h += '<div class="kucuk-not">' + (ogrler ? ogrler.length : 0) + ' öğrenci arasından seçilir.</div>';
    ekran.innerHTML = h;
  }).catch(function (e) { alert('Hata: ' + e.message); });
}
function supaRastgeleCek(kod) {
  kutuSUPA.ogrenciListele(kod).then(function (ogrler) {
    if (!ogrler || !ogrler.length) { alert('Öğrenci yok.'); return; }
    var secilen = ogrler[Math.floor(Math.random() * ogrler.length)];
    var el = document.getElementById('rsSonuc');
    if (el) el.textContent = '🎯 ' + secilen.adsoyad;
  }).catch(function (e) { alert('Hata: ' + e.message); });
}

/* ---- SINIF GENEL RAPORU ---- */
function supaSınıfRapor(kod) {
  if (!kod) return;
  Promise.all([kutuSUPA.ogrenciListele(kod), kutuSUPA.yoklamaTarihe(''), kutuSUPA.sinavListele(kod)])
    .then(function (rz) {
      var ogrler = rz[0] || [];
      var yoklar = rz[1] || [];
      var snv = rz[2] || [];
      var snvOrt = {};
      var beklenti = snv.map(function (s) {
        return kutuSUPA.sinavNotListele(s.id).then(function (n) { return { s: s, n: n || [] }; });
      });
      Promise.all(beklenti).then(function (diziler) {
        ...
        h += '</table></div>';
        if (snv.length) {
          h += '<div class="baslik" style="margin-top:10px"><h2>📝 Sınav Ortalamaları</h2></div>';
          snv.forEach(function (s, i) {
            var ort = diziler[i]._ort.sinifOrt;
            var notSayisi = (diziler[i].n || []).length;
            h += '<div class="ozet-karti" style="margin-bottom:6px"><b>' + esc(s.ad) + '</b> · Sınıf ortalaması: <b>' + (ort == null ? '—' : ort) + '</b> (' + notSayisi + ' not)</div>';
          });
        }
        h += '<div class="kucuk-not" style="margin-top:6px">Toplam yoklama kaydı: ' + (yoklar ? yoklar.length : 0) + '</div>';
        h += '<div style="margin-top:10px;display:flex;gap:6px;flex-wrap:wrap;justify-content:center"><button class="kucuk-buton" style="background:#0984e3" onclick="supaSınıfRaporCSV(supaDeger(\'supaSinifKod\'))">⬇️ CSV İndir</button><button class="btn btn-mor" onclick="window.print()">🖨️ Yazdır / Karne</button></div>';
        ekran.innerHTML = h;
      }).catch(function (e) { alert('Hata: ' + e.message); });
    }).catch(function (e) { alert('Hata: ' + e.message); });
}
function supaSınıfRaporCSV(kod) {
  Promise.all([kutuSUPA.ogrenciListele(kod), kutuSUPA.yoklamaTarihe('')]).then(function (rz) {
    var ogrler = rz[0] || [], yoklar = rz[1] || [];
    var satirlar = ogrler.map(function (o) {
      var ys = yoklar.filter(function (y) { return y.ogrenci_id === o.id; });
      ...
      var toplam = t.H + t.G + t.I + t.Y || 1;
      var yuzde = Math.round((t.H + t.G) / toplam * 100);
      return [o.no || '', o.adsoyad, t.H, t.G, t.I, t.Y, yuzde + '%'];
    });
    supayaIndir('sinif-rapor-' + kod + '.csv', satirlar, ['No', 'Öğrenci', 'Hazır', 'Geç', 'İzinli', 'Yok', 'Devam %']);
  }).catch(function (e) { alert('Hata: ' + e.message); });
}
...
function supaDuyuru(kod) {
  ...
  Promise.all([kutuSUPA.duyuruListele(kod), kutuSUPA.ogrenciListele(kod)])
    .then(function (rz) {
      ...
      var ogrler = rz[1] || [];
      var h = '<button class="geri" onclick="git(\'sinifYonet\')">⬅ Sınıf</button>';
      h += '<div class="baslik"><h1>📢 Sınıf Duyuruları</h1><p>Bağlı öğrenciler anasayfalarında görür.</p></div>';
      h += '<div id="supaMesaj" style="display:none;border-radius:8px;padding:10px;margin-bottom:12px"></div>';
      h += '<div class="ozet-karti"><input id="dyBaslik" type="text" placeholder="Duyuru başlığı" style="padding:8px;border:1px solid #ccc;border-radius:8px;margin:4px 0;width:100%">'
        + '<textarea id="dyMetin" rows="3" placeholder="Duyuru metni" style="width:100%;padding:8px;border:1px solid #ccc;border-radius:8px;font-family:inherit"></textarea>'
        + '<button class="kucuk-buton" style="background:#1f8a70" onclick="supaDuyuruYaz(\'' + kod + '\')">📢 Yayınla</button></div>';
      if (!list || !list.length) h += '<div class="kucuk-not">Henüz duyuru yok.</div>';
      else {
        var beklenti = [];
        list.forEach(function (d) { beklenti.push(kutuSUPA.duyuruOkunma(d.id)); });
        Promise.all(beklenti).then(function (okunmalar) {
          list.forEach(function (d, i) {
            var okundu = okunmalar[i] || [];
            var kalan = ogrler.filter(function (o) { return okundu.indexOf(o.id) === -1; });
            h += '<div class="ozet-karti" style="margin-bottom:6px"><b>📢 ' + esc(d.baslik) + '</b><br>' + esc(d.metin || '') + '<br><small style="color:#888">' + (d.olusturma ? new Date(d.olusturma).toLocaleString('tr-TR') : '') + '</small>'
              + '<br><span style="font-size:12px">👀 Okundu: <b>' + okundu.length + '/' + (ogrler.length || okundu.length) + '</b>'
              + (kalan.length ? ' · Henüz okumayan: ' + kalan.map(function (o) { return esc(o.adsoyad); }).join(', ') : '')
              + '</span> '
              + '<button class="kucuk-buton" style="background:#e05656" onclick="supaDuyuruKaldir(' + d.id + ',\'' + encodeURIComponent(d.kod) + '\')">Sil</button></div>';
          });
          ekran.innerHTML = h;
        }).catch(function () { ekran.innerHTML = h + '<div class="kucuk-not">Okunma bilgisi yüklenemedi.</div>'; });
      }
    }).catch(function (e) { alert('Hata: ' + e.message); });
}
function supaDuyuruYaz(kod) {
  var b = document.getElementById('dyBaslik'), m = document.getElementById('dyMetin');
  if (!b || !b.value.trim()) { alert('Başlık gir.'); return; }
...
function supaKarnemGoster() {
  var bagli = supaJsonOku('supaBagli');
  if (!bagli || !bagli.id) { alert('Önce sınıfa bağlan.'); return; }
  Promise.all([kutuSUPA.sinifGetir(bagli.kod), kutuSUPA.notListele(bagli.id), kutuSUPA.detayListele(bagli.id), kutuSUPA.ogrenciAll(bagli.id), kutuSUPA.testListele(bagli.kod)])
    .then(function (rz) {
      var snf = rz[0] || {};
      var notlar = rz[1] || [];
      var rd = rz[2] || [];
      var all = rz[3] || { yoklama: [], davranis: [] };
      var testler = rz[4] || [];
      var devamsizlik = rd.filter(function (r) { return r.tip === 'devamsizlik'; });
      var toplamDevamGun = devamsizlik.reduce(function (t, r) { return t + (parseFloat(String(r.deger || '0').replace(',', '.')) || 0); }, 0);
      var yoklar = all.yoklama.filter(function (y) { return y.durum === 'Y'; });
      var hazir = all.yoklama.filter(function (y) { return y.durum === 'H' || y.durum === 'G'; });
      var davTotal = (all.davranis || []).reduce(function (t, d) { return t + (d.puan || 0); }, 0);
      var h = '<button class="geri" onclick="git(\'sbaglan\')">⬅ Sınıf</button>';
      h += '<div class="baslik"><h1>🎓 Karnem</h1><p>Yazdırıp ailene gösterebilirsin.</p></div>';
      h += '<div id="karnIcerik" class="ozet-karti" style="border:2px solid #dfe5f0;padding:16px">';
      h += '<div style="text-align:center;margin-bottom:10px"><div style="font-size:18px;font-weight:800">KARNE</div>'
        + '<div style="color:#666;font-size:12px">' + esc((snf && snf.okul) || '') + ' · ' + esc((snf && snf.sinif) || '') + '</div></div>';
      h += '<table style="width:100%;font-size:13px;border-collapse:collapse"><tr><td style="padding:4px">Öğrenci:</td><td><b>' + esc(bagli.adsoyad) + '</b></td>'
        + '<td style="padding:4px">Okul No:</td><td><b>' + esc(bagli.no || '') + '</b></td></tr></table>';
      h += '<div class="baslik"><h2>📝 Karne Notları</h2></div>';
      if (!notlar.length) h += '<div class="kucuk-not">Henüz not yok.</div>';
      else {
        var dersler = {};
        notlar.forEach(function (n) { if (!dersler[n.ders]) dersler[n.ders] = []; dersler[n.ders].push(n); });
        var satirlar = '';
        Object.keys(dersler).forEach(function (ders) {
          var kayitlar = dersler[ders];
          var son = kayitlar[kayitlar.length - 1];
          satirlar += '<tr><td style="padding:4px;border-top:1px solid #eee"><b>' + esc(ders) + '</b></td><td style="padding:4px;border-top:1px solid #eee">' + esc(son.not_) + '</td></tr>';
        });
        h += '<table style="width:100%;font-size:13px;border-collapse:collapse">' + satirlar + '</table>';
      }
      h += '<div class="baslik"><h2>📅 Devam</h2></div>';
      h += '<div class="kucuk-not">Toplam devamsızlık: <b>' + (toplamDevamGun || 0) + '</b> gün · İzinsiz: <b>' + yoklar.length + '</b> · Hazır/Geç gün: <b>' + hazir.length + '</b></div>';
      h += '<div class="baslik"><h2>🏅 Davranış</h2></div>';
      h += '<div class="kucuk-not">Davranış puanı: <b>' + davTotal + '</b> (' + (all.davranis || []).length + ' kayıt)</div>';
      if (testler.length) {
        h += '<div class="baslik"><h2>📝 Testler</h2></div>';
        h += '<div class="kucuk-not">Hazırlanan test sayısı: <b>' + testler.length + '</b></div>';
      }
      h += '<div style="text-align:right;margin-top:14px;font-size:12px;color:#666">İmza: __________________</div>';
      h += '</div>';
